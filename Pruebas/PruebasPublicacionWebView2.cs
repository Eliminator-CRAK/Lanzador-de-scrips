...
public sealed class PruebasPublicacionWebView2
{
    ...
    [Fact]
    public void ProyectoPublicaVersion145()
    {
        var proyecto = File.ReadAllText(ObtenerRutaProyecto("LanzadorScripts.csproj"));

        Assert.Contains("<Version>1.4.5</Version>", proyecto, StringComparison.Ordinal);
        Assert.Contains("<AssemblyVersion>1.4.5.0</AssemblyVersion>", proyecto, StringComparison.Ordinal);
        Assert.Contains("<FileVersion>1.4.5.0</FileVersion>", proyecto, StringComparison.Ordinal);
