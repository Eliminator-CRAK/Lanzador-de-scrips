...
public sealed class PruebasPublicacionWebView2
{
    ...
    [Fact]
    public void ProyectoPublicaVersion156()
    {
        var proyecto = File.ReadAllText(ObtenerRutaProyecto("LanzadorScripts.csproj"));

        Assert.Contains("<Version>1.5.6</Version>", proyecto, StringComparison.Ordinal);
        Assert.Contains("<AssemblyVersion>1.5.6.0</AssemblyVersion>", proyecto, StringComparison.Ordinal);
        Assert.Contains("<FileVersion>1.5.6.0</FileVersion>", proyecto, StringComparison.Ordinal);
