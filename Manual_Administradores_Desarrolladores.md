...
El MSI cierra de forma ordenada la variante instalada cuando no hay scripts activos. Si existen ejecuciones, muestra la aplicacion y bloquea la operacion hasta que terminen. Una portable independiente no bloquea el mantenimiento. Las actualizaciones y reparaciones conservan configuracion. La desinstalacion completa elimina solo `Program Files`, `ProgramData`, perfiles WebView2 locales y Registro conocidos; nunca toca auditoria ni artefactos remotos.

La limpieza del MSI y de la portable usa rutas Win32 extendidas. La compilacion crea y elimina un arbol superior a `MAX_PATH`; si la prueba falla, no se genera la distribucion.
...
La Release `v1.7.2` debe contener bytes identicos en ambos proveedores:
