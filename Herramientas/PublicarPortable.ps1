...
if ($versionProductoEsperada -ne '1.8.2' -or $versionArchivoEsperada -ne '1.8.2.0') {
    throw 'La publicacion MSI y portable requiere la version 1.8.2.'
}

$nombreMsiFinal = 'LanzadorScripts-1.8.2-x64.msi'
$nombrePortableFinal = 'LanzadorScripts_Portable-1.8.2-x64.exe'
...
function Assert-PublishedMsi {
    ...
    try {
        ...
        if ($propiedades.ProductVersion -ne $versionProductoEsperada -or
            ...
            throw 'Los metadatos del MSI publicado no coinciden con el contrato 1.8.2.'
        }
    }
    finally {
        if ($null -ne $vista) {
