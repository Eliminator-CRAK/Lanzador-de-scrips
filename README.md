...
Aplicacion WPF para autorizar y ejecutar scripts PowerShell, BAT y CMD desde una interfaz WebView2 local. La version actual es `1.7.2`.
...
- La limpieza nativa admite rutas superiores a `MAX_PATH` y se valida durante cada publicacion.
...
```powershell
msiexec /i LanzadorScripts-1.7.2-x64.msi
```

Instalacion silenciosa sin acceso de escritorio:

```powershell
msiexec /i LanzadorScripts-1.7.2-x64.msi /qn /norestart
```

Instalacion silenciosa con acceso de escritorio:

```powershell
msiexec /i LanzadorScripts-1.7.2-x64.msi CREATE_DESKTOP_SHORTCUT=1 /qn /norestart
```

Reparacion y desinstalacion:

```powershell
msiexec /fa LanzadorScripts-1.7.2-x64.msi /qn /norestart
msiexec /x LanzadorScripts-1.7.2-x64.msi /qn /norestart
```
...
Durante una reparacion, actualizacion o desinstalacion, el MSI cierra ordenadamente la aplicacion instalada cuando no hay scripts activos. Si hay una ejecucion, muestra un aviso y exige terminarla antes de continuar; una portable independiente no bloquea la operacion. Las actualizaciones y reparaciones conservan configuracion. Una desinstalacion completa elimina solo las rutas locales conocidas y la asociacion.
